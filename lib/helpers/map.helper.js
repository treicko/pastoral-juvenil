...
class MapHelper {
  constructor(map) {
    ...
    this.markersDetail = [];
    this.infoWindowsDetail = [];
  }

  addCurrentLocationControl(entityLocationForm, hasInscriptionArea = false) {
    if (!this.controls['currentLocation'] && this.currentLocation) {
      ...
      controlCurrentLocation.addEventListener('click', () => {
        this.setEventOnCurrentLocationMarker(entityLocationForm, hasInscriptionArea);
      });
  ...
  addEventLocationMarkerOnClick(entityLocationForm, isEvent = false) {
    ...
    searchBtn.onclick = () => {
      ...
      geocoder.geocode({ address: location }, (results, status) => {
        if (status === 'OK') {
          ...
          this.setInfoWindow(`<div align="center"><strong>Ubicacion del evento</strong>
          <br>${results[0].formatted_address}</div>`, this.markers['eventLocation']);
        } else {
  ...
  addSearchBoxEventListenerPlacesChanged(searchBox, entityLocationForm) {
    searchBox.addListener('places_changed', () => {
      ...
      places.forEach((place) => {
        ...
        this.markers['eventLocation'] = marker;
        if (this.circle) {
          const eventRadius = this.circle.radius;
          this.clearCircleShape();
  ...
  clearEventLocationMarker() {
    ...
    if (this.markersDetail.length) {
      this.markersDetail.forEach(marker => marker.setMap(null));
      this.markersDetail = [];
    }
  }

  clearInfoWindowDetail() {
    if (this.infoWindowsDetail.length) {
      this.infoWindowsDetail.forEach(infoWindow => infoWindow.close());
    }
  }
  ...
  setEventOnCurrentLocationMarker(entityLocationForm, hasInscriptionArea) {
    this.clearEventLocationMarker();
  ...
  setEntityOnMapForShow(entity, isEvent = false) {
    this.clearEventLocationMarker();
    this.clearCircleShape();
  ...
  setEntitiesDetailOnMapForShow(entitiesDetail) {
    this.clearEventLocationMarker();
    this.clearCircleShape();
    this.clearInfoWindowDetail();
    ...
    this.markersDetail = entitiesDetail.map((entity) => {
      const marker = new google.maps.Marker({
        position: new google.maps.LatLng(entity.location.lat, entity.location.lng),
        label: entity.label,
        anchorPoint: new google.maps.Point(0, -29),
      ...
      geocoder.geocode({ location: marker.getPosition() }, (results, status) => {
        if (status === 'OK') {
          infoWindow.setContent(`<div align="center"><strong>${entity.name}</strong><br>${results[0].formatted_address}<br>${entity.link}</div>`, marker);
        }
      });
      ...
      this.infoWindowsDetail.push(infoWindow);
    ...
    */
    ...
    /* this.markersDetail.forEach((entity) => {
    }); */

    if (this.markersDetail.length) {
      this.map.instance.setCenter(this.markersDetail[0].getPosition());
      this.map.instance.setZoom(15);
    }
  }
